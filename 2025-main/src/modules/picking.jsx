...
import {
  Navigation, Route, MapPin, Clock, Package, Users, TrendingUp,
  Zap, CheckCircle, AlertCircle, Play, Pause, RotateCcw, Target,
  ArrowRight, ArrowDown, Layers, Gauge, Timer, Activity, Settings,
  Scan, Camera, Volume2, VolumeX, Smartphone, Lightbulb, Shield,
  BarChart3, PieChart, Map, Warehouse, ShoppingCart, Star, Flag,
  Eye, EyeOff, RefreshCw, Download, Filter, Search, MoreVertical,
  ChevronRight, ChevronDown, Plus, Minus, Edit, Save, X, Check
} from 'lucide-react';
...
const generatePickingRoutes = () => [
  {
    ...
    efficiency: 94,
    orders: [
      { id: 'MIA001234', items: ['Vali Larita 28"', 'Luggage Tag'], positions: ['A-08', 'B-03'], priority: 'P1' },
      { id: 'MIA001235', items: ['Vali Cover'], positions: ['B-05'], priority: 'P2' },
      { id: 'MIA001236', items: ['Travel Pillow', 'TSA Lock'], positions: ['B-09', 'B-12'], priority: 'P3' }
    ],
    optimizedPath: [
      { zone: 'A', position: '08', item: 'Vali Larita 28"', status: 'completed', time: 2.5 },
      { zone: 'A', position: '12', item: 'Vali Pisani 24"', status: 'current', time: 3.1 },
      { zone: 'B', position: '03', item: 'Luggage Tag', status: 'pending', time: 1.8 },
      { zone: 'B', position: '05', item: 'Vali Cover', status: 'pending', time: 2.2 },
      { zone: 'B', position: '09', item: 'Travel Pillow', status: 'pending', time: 1.9 }
    ],
    suggestions: [
  ...
  },
  {
    ...
    efficiency: 0,
    orders: [
      { id: 'MIA001237', items: ['Premium Luggage Set'], positions: ['D-01'], priority: 'P1' },
      { id: 'MIA001238', items: ['Business Tag', 'Travel Kit'], positions: ['D-03', 'B-15'], priority: 'P2' }
    ],
    optimizedPath: [
      { zone: 'D', position: '01', item: 'Premium Luggage Set', status: 'pending', time: 4.2 },
      { zone: 'D', position: '03', item: 'Business Tag', status: 'pending', time: 2.1 },
      { zone: 'B', position: '15', item: 'Travel Kit', status: 'pending', time: 2.8 }
    ],
    suggestions: [
  ...
  },
  {
    ...
    efficiency: 91,
    orders: [
      { id: 'MIA001239', items: ['Vali Set 3 pieces'], positions: ['A-15', 'A-17', 'A-19'], priority: 'P3' },
      { id: 'MIA001240', items: ['Multiple accessories'], positions: ['B-01', 'B-07', 'B-11'], priority: 'P4' }
    ],
    optimizedPath: [
      { zone: 'A', position: '15', item: 'Vali Large', status: 'completed', time: 3.8 },
      { zone: 'A', position: '17', item: 'Vali Medium', status: 'completed', time: 3.2 },
      { zone: 'A', position: '19', item: 'Vali Small', status: 'completed', time: 2.9 },
      { zone: 'B', position: '01', item: 'Accessory 1', status: 'completed', time: 1.5 },
      { zone: 'B', position: '07', item: 'Accessory 2', status: 'completed', time: 1.8 },
      { zone: 'B', position: '11', item: 'Accessory 3', status: 'completed', time: 2.1 }
    ],
    suggestions: []
  }
];
...
const SmartPickingSystem = () => {
  ...
  const [pickingMode, setPickingMode] = useState('batch'); // single, batch, zone
    ...
    setRoutes(prev => prev.map(route => {
      if (route.id === routeId) {
        switch (action) {
          ...
          case 'optimize':
            return { ...route, efficiency: Math.min(route.efficiency + 5, 99) };
          default:
            return route;
        }
      }
  ...
  const generateOptimizedRoute = () => {
    ...
    const newRoute = {
      ...
      efficiency: 95,
      orders: [
        { id: 'MIA00' + Math.floor(Math.random()*9999), items: ['Auto-optimized route'], positions: ['A-05'], priority: 'P1' }
      ],
      optimizedPath: [
        { zone: 'A', position: '05', item: 'Optimized Item', status: 'pending', time: 2.1 }
      ],
...
// ==================== ROUTES VIEW ====================
const RoutesView = ({ routes, selectedRoute, onSelectRoute, onRouteAction, showOptimizationPanel, setShowOptimizationPanel }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="space-y-6">
        {selectedRoute && (
          <RouteDetails route={selectedRoute} onAction={(action) => onRouteAction(selectedRoute.id, action)} />
        )}
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex space-x-2">
            ...
            <button
              onClick={(e) => {e.stopPropagation(); onAction('optimize');}}
        ...
        <div className="mb-4">
          <div className="flex justify-between text-sm mb-1">
            <span>Progress: {route.completedItems}/{route.totalItems} items</span>
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div className="text-center">
            ...
            <p className="font-medium">{route.currentDistance}/{route.totalDistance}m</p>
          </div>
          <div className="text-center">
          ...
          <div className="text-center">
            ...
            <p className="font-medium">
              {route.estimatedCompletion
                ? new Date(route.estimatedCompletion).toLocaleTimeString('vi-VN', {hour: '2-digit', minute: '2-digit'})
                : 'N/A'
              }
            </p>
          </div>
          ...
          <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm">
                Hiện tại: <strong>{route.currentLocation}</strong> → Tiếp theo: <strong>{route.nextLocation}</strong>
              </span>
            </div>
          </div>
        )}
          ...
          {route.optimizedPath.map((step, index) => (
            <div
              ...
              className={`flex items-center space-x-3 p-3 rounded-lg border ${
                ...
                step.status === 'current' ? 'bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800' :
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center space-x-2">
                  <span className="font-medium">{step.zone}-{step.position}</span>
          ...
          {route.orders.map((order, index) => (
            <div key={index} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              ...
              <p className="text-xs text-gray-500">
                Positions: {order.positions.join(' → ')}
              </p>
            </div>
          ))}
        </div>
          ...
          <div className="space-y-3">
            {Object.entries(optimizationSettings).map(([key, value]) => (
              <label key={key} className="flex items-center space-x-3">
                ...
                />
                <span className="text-sm">
                  {key === 'prioritizeP1' ? 'Ưu tiên đơn P1 trước' :
                   key === 'minimizeDistance' ? 'Tối thiểu hóa khoảng cách di chuyển' :
                   key === 'balanceWorkload' ? 'Cân bằng khối lượng công việc' :
                   key === 'considerItemWeight' ? 'Xem xét trọng lượng sản phẩm' :
        ...
        <div className="grid grid-cols-4 gap-8 h-full">
          {Object.entries(WAREHOUSE_ZONES).map(([zone, info]) => (
            <div key={zone} className="relative">
              <div className={`w-full h-32 ${info.color} bg-opacity-20 border-2 border-dashed rounded-lg flex items-center justify-center`}>
                <div className="text-center">
            ...
            {Object.entries(metrics.overall).map(([key, value]) => (
              <div key={key} className="flex justify-between items-center">
                ...
                <span className="font-medium">
                  ...
                  {key.includes('Rate') || key.includes('efficiency') || key.includes('Optimization') ? '%' :
