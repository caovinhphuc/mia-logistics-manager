...
/**
 * Script đơn giản để kiểm tra Google Sheets
 */
...
async function checkGoogleSheets() {
  try {
    console.log('🔍 Kiểm tra Google Sheets...');
    ...
    // Use the correct method for v5
    await doc.auth.useServiceAccountAuth(serviceAccount);
    await doc.loadInfo();
    ...
    if (usersSheet) {
      ...
      // Get first few rows
      const rows = await usersSheet.getRows({ limit: 3 });
      console.log('\n📊 Sample Data (first 3 rows):');

      rows.forEach((row, index) => {
        console.log(`\nRow ${index + 1}:`);
        usersSheet.headerValues.forEach(header => {
