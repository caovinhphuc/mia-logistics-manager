...
start_services() {
    ...
    mkdir -p logs
    node backend/server.cjs > logs/backend-startup.log 2>&1 &
    local backend_pid=$!

    # Wait for backend to start
    echo -e "${BLUE}Waiting for backend to start...${NC}"
    local retries=20
    until curl -s -f http://localhost:5050/health > /dev/null 2>&1; do
        sleep 1
        retries=$((retries-1))
        if [ $retries -le 0 ]; then
            echo -e "${RED}❌ Backend failed to start (see logs/backend-startup.log)${NC}"
            return 1
        fi
    done
...
kill_process "backend/server.cjs" "Backend server"
