// Script fix Google Sheets methods
const fs = require('fs');
const path = require('path');

console.log('🔧 FIX GOOGLE SHEETS METHODS');
...
if (fs.existsSync(proxyServicePath)) {
  ...
  if (content.includes('mapCategoryToType(category)')) {
    console.log('✅ ProxyLocationsService đã có method mapCategoryToType');
  } else {
    console.log('❌ ProxyLocationsService chưa có method mapCategoryToType');
  }

  if (content.includes('buildFullAddress(locationData)')) {
    console.log('✅ ProxyLocationsService đã có method buildFullAddress');
  } else {
    console.log('❌ ProxyLocationsService chưa có method buildFullAddress');
  }
...
console.log('=' .repeat(50));
console.log('✅ Script fix Google Sheets methods hoàn thành!');
