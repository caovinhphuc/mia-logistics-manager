...
import { ModernPageLayout } from '../../../components/layout';
...
const VolumeCalculator: React.FC = () => {
  ...
  const totalVolume = useMemo(() => {
    return rules.reduce((sum, r) => sum + (counts[r.id] || 0) * (r.unitVolume || 0), 0);
  }, [rules, counts]);
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            {editMode && hasPermission('shipments', 'delete') && (
              <Tooltip title="Xóa dòng">
                <IconButton
                  color="error"
                  onClick={() => deleteRow(rules.findIndex((r) => r.id === row.id))}
                >
                  <DeleteIcon />
                </IconButton>
              </Tooltip>
