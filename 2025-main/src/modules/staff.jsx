...
import {
  Users, Calendar, Clock, Target, TrendingUp, TrendingDown, Award,
  BarChart3, Activity, CheckCircle, AlertTriangle, Star, Flag,
  User, UserPlus, UserCheck, UserX, Coffee, Moon, Sun, Settings,
  Edit, Save, X, Plus, Minus, Eye, EyeOff, Search, Filter,
  Download, RefreshCw, Mail, Smartphone, MapPin, Timer, Zap,
  Package, Truck, Route, Layers, Database, FileText, Bell,
  ChevronDown, ChevronRight, MoreVertical, Share2, Upload,
  PlayCircle, PauseCircle, Square, RotateCcw, ArrowUp, ArrowDown
} from 'lucide-react';
...
const generateStaffData = () => ({
  ...
  staff: [
    ...
    },
    {
      ...
      skills: ['Leadership', 'Training', 'Quality Control', 'Process Improvement'],
      certifications: ['Safety Level 3', 'Leadership Certification', 'Quality Management'],
      schedule: {
        monday: { start: '08:00', end: '17:00', break: '12:00-13:00' },
        tuesday: { start: '08:00', end: '17:00', break: '12:00-13:00' },
        wednesday: { start: '08:00', end: '17:00', break: '12:00-13:00' },
      ...
      recentActivity: [
        { time: '15:00', action: 'Team meeting completed', zone: 'Meeting Room' },
    ...
    },
    {
      ...
      recentActivity: [
        { time: '14:15', action: 'Picking practice session', zone: 'Training Room' },
        { time: '11:00', action: 'Safety training module 3', zone: 'Training Room' },
        { time: '09:30', action: 'Mentor assignment with NV001', zone: 'Zone A' }
      ]
    }
  ],
  ...
  tasks: [
    {
      ...
      description: 'Complete cycle count for all items in Zone A sections A1-A12'
    },
    {
      id: 'TSK002',
...
const StaffManagement = () => {
  ...
  const filteredStaff = useMemo(() => {
    return data.staff.filter(staff => {
      if (filters.department !== 'all' && staff.department !== filters.department) return false;
      if (filters.shift !== 'all' && staff.currentShift !== filters.shift) return false;
      if (filters.status !== 'all' && staff.status !== filters.status) return false;
      if (filters.level !== 'all' && staff.level !== filters.level) return false;
      return true;
    });
  }, [data.staff, filters]);
      ...
      <div className={`${themeClasses.surface} border-b ${themeClasses.border} p-6`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">Staff Management & Scheduling</h1>
              <p className={`${themeClasses.text.muted}`}>
                Quản lý nhân sự kho vận • 01/06/2025 14:50 • {data.summary.activeToday}/{data.summary.totalStaff} active
              </p>
            </div>
          </div>
          ...
          <div key={index} className={`${themeClasses.surface} rounded-xl border ${themeClasses.border} p-4 hover:shadow-md transition-shadow`}>
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <p className={`text-sm ${themeClasses.text.muted} mb-1`}>{card.label}</p>
            ...
            {data.shifts.map((shift) => (
              <div key={shift.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <div className="text-right">
                  <p className="font-medium">{shift.currentStaff}/{shift.capacity}</p>
                  <div className="w-20 bg-gray-200 dark:bg-gray-600 rounded-full h-2 mt-1">
                    <div
                      className={`h-2 rounded-full ${
                        (shift.currentStaff / shift.capacity) >= 0.8 ? 'bg-green-500' :
                        (shift.currentStaff / shift.capacity) >= 0.6 ? 'bg-yellow-500' : 'bg-red-500'
                      }`}
                      style={{ width: `${(shift.currentStaff / shift.capacity) * 100}%` }}
                    ></div>
                  </div>
                </div>
              </div>
...
// ==================== STAFF VIEW ====================
const StaffView = ({ staff, filters, setFilters, selectedStaff, setSelectedStaff, themeClasses }) => {
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{member.todayHours}/{member.plannedHours}h</span>
              </div>
              ...
              <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                <div className="grid grid-cols-3 gap-2 text-center text-xs">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{member.performance.ordersCompleted}</p>
                  </div>
                </div>
              </div>
            </div>
...
const StaffDetailModal = ({ staff, onClose, themeClasses }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className={`${themeClasses.surface} rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto`}>
        {/* Header */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">{staff.id} • Joined {staff.joinDate}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <p className="text-2xl font-bold text-blue-600">{staff.performance.efficiency}%</p>
                <p className="text-sm text-gray-600">Efficiency</p>
              </div>
              <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                <p className="text-2xl font-bold text-green-600">{staff.performance.sla}%</p>
                <p className="text-sm text-gray-600">SLA Compliance</p>
              </div>
              <div className="text-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                <p className="text-2xl font-bold text-purple-600">{staff.performance.ordersCompleted}</p>
                <p className="text-sm text-gray-600">Orders Today</p>
              </div>
              <div className="text-center p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                <p className="text-2xl font-bold text-orange-600">{staff.performance.avgTimePerOrder}m</p>
                <p className="text-sm text-gray-600">Avg Time/Order</p>
              </div>
              <div className="text-center p-4 bg-teal-50 dark:bg-teal-900/20 rounded-lg">
                <p className="text-2xl font-bold text-teal-600">{staff.performance.accuracy}%</p>
                <p className="text-sm text-gray-600">Accuracy</p>
              </div>
              <div className="text-center p-4 bg-red-50 dark:bg-red-900/20 rounded-lg">
                <p className="text-2xl font-bold text-red-600">{staff.performance.errorCount}</p>
    ...
    <div className="space-y-6">
            ...
            <div key={shift.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <div className="flex justify-between items-center mb-3">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  (shift.currentStaff / shift.capacity) >= 0.8 ? 'bg-green-100 text-green-700' :
                  (shift.currentStaff / shift.capacity) >= 0.6 ? 'bg-yellow-100 text-yellow-700' :
                  'bg-red-100 text-red-700'
                }`}>
                  {Math.round((shift.currentStaff / shift.capacity) * 100)}% capacity
                </span>
              </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{shift.currentStaff}/{shift.capacity}</span>
                </div>
              </div>
      ...
      <div className={`${themeClasses.surface} rounded-xl border ${themeClasses.border} p-6`}>
        ...
        <p className="text-gray-500 mb-4">
          💡 Interactive calendar với drag-drop scheduling sẽ được tích hợp trong phiên bản production
        </p>
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-8 text-center">
          <Calendar size={48} className="mx-auto mb-4 text-gray-400" />
          <p className="text-gray-600 dark:text-gray-300">
            Weekly calendar view với khả năng assign staff vào shifts, manage time-off requests, và optimize coverage
          </p>
        </div>
      </div>
    </div>
            ...
            <div key={program.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{program.type} • {program.duration}</p>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-3">
                <div>
                  ...
                  <span className="font-medium ml-1">{program.participants}</span>
                </div>
                <div>
...
const TasksView = ({ tasks, themeClasses }) => {
  const getPriorityBadge = (priority) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs font-medium border ${badges[priority]}`}>
            ...
            <div key={task.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="font-medium ml-1">{Math.round(task.estimatedTime/60)}h</span>
                </div>
                <div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {Object.entries(data.trends).map(([key, value]) => (
          <div key={key} className={`${themeClasses.surface} rounded-xl border ${themeClasses.border} p-4`}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</p>
                <p className="text-2xl font-bold">{value}</p>
              </div>
              <div className={`p-2 rounded-lg ${value.startsWith('+') ? 'bg-green-100' : 'bg-red-100'}`}>
                {value.startsWith('+') ? (
