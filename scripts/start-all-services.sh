...
start_service "Backend API" 5050 "cd backend && npm start"
if [ $? -eq 0 ]; then
    wait_for_service "http://localhost:5050/health" "Backend API"
fi
...
echo "API Status: http://localhost:5050/api/status"
echo ""
