...
check_ports() {
    ...
    if lsof -Pi :5050 -sTCP:LISTEN -t >/dev/null ; then
        print_warning "Port 5050 is already in use. Backend may fail to start."
    else
        print_success "Port 5050 available for Backend ✅"
    fi
}
...
start_backend() {
    print_status "Starting Backend (Node.js) on port 5050..."
    ...
    echo "🖥️  Backend URL: http://localhost:5050"
}
