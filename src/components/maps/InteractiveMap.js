    ...
    try {
      ...
      if (data.length > 0) {
        ...
        if (bounds.lat.length > 0) {
          const centerLat =
            bounds.lat.reduce((a, b) => a + b, 0) / bounds.lat.length;
          const centerLng =
            bounds.lng.reduce((a, b) => a + b, 0) / bounds.lng.length;
          ...
          <ButtonGroup orientation="vertical" size="small">
            {Object.entries(locationLabels).map(([type, label]) => (
              <Button
                key={type}
                variant={
                  activeFilters.includes(type) ? "contained" : "outlined"
                }
                startIcon={
                  <span>
                    {locationIcons[type].options.html.match(/>(.*?)</)?.[1]}
                  </span>
                }
