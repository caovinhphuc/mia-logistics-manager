...
import {
  ...
  BarChart3, PieChart, Target, Zap, Clock, DollarSign, Percent,
  Search, Filter, Download, RefreshCw, Settings, Plus, Edit,
  ArrowUp, ArrowDown, Minus, Eye, EyeOff, Star, Flag, Award,
  Truck, MapPin, Calendar, Timer, Activity, Users, Layers,
  Database, Warehouse, ShoppingCart, FileText, Mail, Bell,
  Coffee, Moon, Sun, MoreVertical, ChevronDown, ChevronRight,
  X, Check, Save, Upload, Scan, Camera, Globe, Smartphone
} from 'lucide-react';
...
const generateInventoryData = () => ({
  ...
  forecasting: {
    nextWeek: [
      { sku: 'VALI-LAR-28', predicted: 16, confidence: 92, trend: 'increasing' },
      { sku: 'TAG-TS-001', predicted: 39, confidence: 89, trend: 'stable' },
      { sku: 'VALI-PIS-24', predicted: 13, confidence: 85, trend: 'decreasing' },
      { sku: 'NECK-PIL-001', predicted: 8, confidence: 94, trend: 'stable' },
      { sku: 'LOCK-TSA-001', predicted: 24, confidence: 91, trend: 'increasing' }
    ],
    insights: [
  ...
  movements: [
    { id: 'IN001', type: 'in', sku: 'TAG-TS-001', qty: 100, timestamp: '2025-06-01 14:20', reason: 'Restock', user: 'Nguyễn Văn A' },
    { id: 'OUT001', type: 'out', sku: 'VALI-LAR-28', qty: 2, timestamp: '2025-06-01 14:15', reason: 'Sale', user: 'System' },
    { id: 'ADJ001', type: 'adjustment', sku: 'LOCK-TSA-001', qty: -1, timestamp: '2025-06-01 13:45', reason: 'Damage', user: 'Trần Thị B' },
    { id: 'OUT002', type: 'out', sku: 'NECK-PIL-001', qty: 1, timestamp: '2025-06-01 13:30', reason: 'Sale', user: 'System' },
    { id: 'IN002', type: 'in', sku: 'VALI-LAR-28', qty: 5, timestamp: '2025-06-01 12:00', reason: 'Return', user: 'Lê Văn C' }
  ],

  alerts: [
    { type: 'critical', message: 'VALI-PIS-24 out of stock - 3 đơn hàng pending', action: 'Emergency reorder' },
    { type: 'warning', message: 'VALI-LAR-28 sẽ hết stock trong 3 ngày', action: 'Schedule reorder' },
    { type: 'info', message: 'NECK-PIL-001 overstock - consider promotion', action: 'Review pricing' },
    { type: 'opportunity', message: 'TSA locks trending up +25% - increase order', action: 'Boost inventory' }
  ]
});
...
const InventoryManagement = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setData(prev => ({
        ...
        summary: {
          ...prev.summary,
          totalValue: prev.summary.totalValue + Math.random() * 1000000 - 500000
        }
      }));
    }, 10000);
  ...
  const filteredProducts = useMemo(() => {
    return data.products.filter(product => {
      ...
      if (filters.stockStatus !== 'All' && product.stockStatus !== filters.stockStatus) return false;
      if (filters.abcClass !== 'All' && product.abc !== filters.abcClass) return false;
    ...
    <div className={`min-h-screen transition-colors duration-200 ${themeClasses.background} ${themeClasses.text.primary}`}>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className={`${themeClasses.text.muted}`}>
                Quản lý tồn kho thông minh • 01/06/2025 14:45 • {data.summary.totalSKUs} SKUs
              </p>
            </div>
          </div>
      ...
      <div className="p-6">
        ...
        {activeView === 'forecasting' && (
          <ForecastingView data={data.forecasting} themeClasses={themeClasses} />
        )}
        {activeView === 'movements' && (
          <MovementsView movements={data.movements} themeClasses={themeClasses} />
        )}
        {activeView === 'alerts' && (
...
const OverviewView = ({ data, themeClasses }) => {
  ...
  const stockStatusData = [
    { status: 'Optimal', count: data.summary.totalSKUs - data.summary.lowStockItems - data.summary.outOfStockItems - data.summary.overstockItems, color: 'bg-green-500' },
    { status: 'Low Stock', count: data.summary.lowStockItems, color: 'bg-yellow-500' },
    { status: 'Out of Stock', count: data.summary.outOfStockItems, color: 'bg-red-500' },
    { status: 'Overstock', count: data.summary.overstockItems, color: 'bg-blue-500' }
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {kpiCards.map((kpi, index) => (
          <div key={index} className={`${themeClasses.surface} rounded-xl border ${themeClasses.border} p-4 hover:shadow-md transition-shadow`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={`${themeClasses.surface} rounded-xl border ${themeClasses.border} p-6`}>
          <h3 className="text-lg font-semibold mb-4">Critical Actions Required</h3>
      ...
      {/* ABC Analysis */}
      <div className={`${themeClasses.surface} rounded-xl border ${themeClasses.border} p-6`}>
        <h3 className="text-lg font-semibold mb-4">ABC Analysis & Category Performance</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
            ...
            <p className="text-sm text-gray-600">High value • 20% items • 80% revenue</p>
            <p className="text-xs text-gray-500 mt-1">156 SKUs • 94.2% in stock</p>
          </div>

          <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <Star className="mx-auto mb-2 text-blue-600" size={24} />
            <p className="text-2xl font-bold text-blue-600">Class B</p>
            <p className="text-sm text-gray-600">Medium value • 30% items • 15% revenue</p>
            <p className="text-xs text-gray-500 mt-1">374 SKUs • 96.8% in stock</p>
          </div>

          <div className="text-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <Package className="mx-auto mb-2 text-gray-600" size={24} />
            <p className="text-2xl font-bold text-gray-600">Class C</p>
            <p className="text-sm text-gray-600">Low value • 50% items • 5% revenue</p>
            <p className="text-xs text-gray-500 mt-1">717 SKUs • 91.3% in stock</p>
          </div>
        </div>
      </div>
    </div>
...
// ==================== PRODUCTS VIEW ====================
const ProductsView = ({
  products, filters, setFilters, selectedProducts, setSelectedProducts,
  showBulkActions, setShowBulkActions, onBulkAction, themeClasses
}) => {
  ...
  return (
    <div className="space-y-6">
      {/* Filters */}
      <div className={`${themeClasses.surface} rounded-xl border ${themeClasses.border} p-4`}>
        <div className="flex flex-wrap items-center gap-4">
          ...
          <select
            ...
            onChange={(e) => setFilters(prev => ({ ...prev, abcClass: e.target.value }))}
            className="px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
          >
            {ABC_CLASSES.map(cls => (
              <option key={cls} value={cls}>{cls === 'All' ? 'Tất cả ABC' : `Class ${cls}`}</option>
            ))}
          </select>
      ...
      {/* Bulk Actions */}
      {showBulkActions && (
        <div className={`${themeClasses.surface} rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 p-4`}>
          <div className="flex items-center justify-between">
      ...
      {/* Products table */}
      <div className={`${themeClasses.surface} rounded-xl border ${themeClasses.border} overflow-hidden`}>
...
// ==================== PRODUCT ROW ====================
const ProductRow = ({ product, isSelected, onSelect, getStatusBadge, themeClasses }) => {
    ...
    <tr className={`hover:bg-gray-50 dark:hover:bg-gray-700 ${isSelected ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
      ...
      <td className="px-6 py-4">
        <div className="flex items-center space-x-3">
          <div className={`w-2 h-8 rounded ${
            product.abc === 'A' ? 'bg-yellow-500' :
            product.abc === 'B' ? 'bg-blue-500' : 'bg-gray-500'
          }`}></div>
          <div>
            <p className="font-medium">{product.name}</p>
            <p className="text-sm text-gray-500">{product.sku} • {product.category}</p>
            <p className="text-xs text-gray-400">{product.location} • Class {product.abc}</p>
          </div>
        </div>
      </td>
      ...
      <td className="px-6 py-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-500">Min: {product.minStock} • Reorder: {product.reorderPoint}</p>
        </div>
      </td>
      ...
      <td className="px-6 py-4">
        <div>
          <p className="font-medium">{Math.round(product.currentStock * product.unitCost / 1000)}K</p>
...
const ForecastingView = ({ data, themeClasses }) => {
  return (
    <div className="space-y-6">
      ...
      {/* Insights */}
      <div className={`${themeClasses.surface} rounded-xl border ${themeClasses.border} p-6`}>
        <h3 className="text-lg font-semibold mb-4">AI Insights & Market Intelligence</h3>
        <div className="space-y-3">
...
const MovementsView = ({ movements, themeClasses }) => {
  const getMovementIcon = (type) => {
    switch (type) {
      case 'in': return <ArrowDown className="text-green-600" size={16} />;
      case 'out': return <ArrowUp className="text-red-600" size={16} />;
      case 'adjustment': return <Edit className="text-blue-600" size={16} />;
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {movement.type === 'in' ? '+' : movement.type === 'out' ? '-' : '±'}{Math.abs(movement.qty)} • {movement.reason}
                  </p>
                </div>
              </div>
              <div className="text-right">
...
const AlertsView = ({ alerts, themeClasses }) => {
  const getAlertIcon = (type) => {
    switch (type) {
      ...
      case 'opportunity': return <TrendingUp className="text-green-600" size={20} />;
  ...
  return (
    <div className="space-y-4">
      {alerts.map((alert, index) => (
        <div key={index} className={`${themeClasses.surface} rounded-xl border p-6 ${getAlertBg(alert.type)}`}>
          <div className="flex items-start justify-between">
...
// ==================== MODULE COMPONENT ====================
const ModuleInventoryManagement = ({ themeClasses }) => (
