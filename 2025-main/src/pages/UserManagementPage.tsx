...
const UserManagementPage: React.FC = () => {
  ...
  // State management
  const [users, setUsers] = useState<UserData[]>([]);  const [filteredUsers, setFilteredUsers] = useState<UserData[]>([]);  const [searchTerm, setSearchTerm] = useState('');
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {/* Statistics Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">        <WidgetWrapper
          widgetId="user-total"
          title="Tổng người dùng"
          description={`${stats.newThisMonth} người dùng mới tháng này`}
          onRefresh={() => {
            setIsLoading(true);
            // Simulate refresh
        ...
        >
          ...
          widgetId="user-active"
          title="Đang hoạt động"
          description={`${((stats.active / stats.total) * 100).toFixed(1)}% tổng số người dùng`}
          onRefresh={() => {
            setIsLoading(true);
            setTimeout(() => setIsLoading(false), 1000);
          }}
        ...
        >
          ...
          widgetId="user-pending"
          title="Chờ duyệt"
          description="Người dùng chờ xác minh và kích hoạt"
          onRefresh={() => {
            setIsLoading(true);
            setTimeout(() => setIsLoading(false), 1000);
          }}
        ...
        >
          ...
          widgetId="user-performance"
          title="Hiệu suất trung bình"
          description="Hiệu suất công việc của tất cả người dùng"
          onRefresh={() => {
            setIsLoading(true);
            setTimeout(() => setIsLoading(false), 1000);
          }}
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-gray-900 dark:text-white">
                {users.length > 0 ? (users.reduce((acc, u) => acc + u.efficiency, 0) / users.length).toFixed(1) : 0}%
              </div>
              <div className="text-sm text-gray-600 dark:text-gray-300">
                Tất cả người dùng
              </div>
      ...
      </WidgetWrapper>      {/* Users List/Grid */}      <WidgetWrapper
        widgetId="user-list"
        title={`Danh sách người dùng (${filteredUsers.length})`}
        description="Quản lý và theo dõi thông tin người dùng"
        onRefresh={() => {
          setIsLoading(true);
          setTimeout(() => setIsLoading(false), 1000);
        }}
