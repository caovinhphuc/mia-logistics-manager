// Advanced Staff Schedule Management Widget - Mobile Optimized
...
// ==================== MOCK DATA ====================
const staff = [
  { id: 1, name: 'Nguyễn Văn A', status: 'active', efficiency: 95, role: 'manager', type: 'fulltime' },
  { id: 2, name: 'Trần Thị B', status: 'active', efficiency: 88, role: 'staff', type: 'contract' },
  { id: 3, name: 'Lê Văn C', status: 'inactive', efficiency: 78, role: 'staff', type: 'fulltime' },
  { id: 4, name: 'Phạm Thị D', status: 'active', efficiency: 85, role: 'staff', type: 'contract' },
  { id: 5, name: 'Hoàng Minh E', status: 'active', efficiency: 90, role: 'staff', type: 'fulltime' }
];

const analytics = {
  top20Staff: [1, 2, 3, 4, 5],
  workloadForecast: [
    { date: '2025-05-26', orders: 120, capacity: 150, required: 8 },
    { date: '2025-05-27', orders: 140, capacity: 150, required: 9 },
    { date: '2025-05-28', orders: 180, capacity: 150, required: 12 },
    { date: '2025-05-29', orders: 200, capacity: 150, required: 14 },
    { date: '2025-05-30', orders: 160, capacity: 150, required: 10 },
    { date: '2025-05-31', orders: 90, capacity: 150, required: 6 }
  ]
};
...
// Mock Components for missing modals
...
// ==================== MAIN COMPONENT ====================
const StaffScheduleWidget = ({ staffData = staff, themeClasses: providedTheme = themeClasses, onScheduleUpdate }) => {
  ...
  const renderScheduleStats = () => (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
      <div className={`p-3 rounded-lg ${providedTheme.surfaceHover}`}>
        <div className="flex items-center gap-2 mb-2">
          <Calendar size={16} className="text-blue-500" />
          <span className={`text-sm font-medium ${providedTheme.text.primary}`}>Tổng ca</span>
        </div>
        <p className={`text-xl font-bold ${providedTheme.text.primary}`}>
  ...
  const renderWeekView = () => (
    <div className="space-y-4">
      <div className="grid grid-cols-8 gap-2">
        <div className={`p-3 font-semibold ${providedTheme.text.primary}`}>Ca làm việc</div>
        {weekDates.map((date, index) => (
          <div key={index} className="text-center">
            <div className={`text-sm ${providedTheme.text.muted}`}>
              {date.toLocaleDateString('vi-VN', { weekday: 'short' })}
            </div>
            <div className={`text-lg font-semibold ${providedTheme.text.primary} ${
      ...
      <div className="space-y-2">
        {['morning', 'afternoon', 'night'].map(shift => (
          <div key={shift} className="grid grid-cols-8 gap-2">
            <div className={`p-3 rounded-lg ${providedTheme.surfaceHover} flex items-center`}>
              <span className={`font-medium ${providedTheme.text.primary}`}>
            ...
            {weekDates.map((date, dateIndex) => {
              ...
              return (
                <div key={dateIndex} className={`min-h-[80px] p-2 rounded-lg border-2 border-dashed ${
                  daySchedules.length > 0 ? providedTheme.border : 'border-gray-200 dark:border-gray-700'
                } ${providedTheme.surface}`}>
                  {daySchedules.map(schedule => (
                    <ShiftCard
                      key={schedule.id}
                      schedule={schedule}
                      themeClasses={providedTheme}
                  ...
                  {daySchedules.length === 0 && (
                    <button
                      ...
                      className={`w-full h-full min-h-[60px] rounded border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-blue-500 transition-colors flex items-center justify-center ${providedTheme.text.muted} hover:text-blue-500`}
                    >
                      <Plus size={20} />
                    </button>
                  )}
  ...
  const AddShiftModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className={`w-full max-w-md p-6 rounded-xl ${providedTheme.surface} ${providedTheme.border} border`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-lg font-semibold ${providedTheme.text.primary}`}>
              Thêm ca làm việc
            </h3>
            <button
              onClick={() => setShowAddShift(false)}
              className={`p-2 rounded-lg ${providedTheme.buttonSecondary} hover:${providedTheme.buttonSecondaryHover} transition-colors`}
            >
              <X size={20} />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className={`block text-sm font-medium ${providedTheme.text.primary} mb-2`}>
                Nhân viên
              </label>
              <select
                value={formData.staffId}
                onChange={(e) => setFormData({ ...formData, staffId: e.target.value })}
                className={`w-full px-3 py-2 rounded-lg border ${providedTheme.input} ${providedTheme.border} focus:outline-none focus:ring-2 focus:ring-blue-500`}
                required
              >
            ...
            <div className="flex gap-3 pt-4">
              <button
                type="button"
                onClick={() => setShowAddShift(false)}
                className={`flex-1 px-4 py-2 rounded-lg border ${providedTheme.border} ${providedTheme.buttonSecondary} hover:${providedTheme.buttonSecondaryHover} transition-colors`}
              >
                Hủy
              </button>
              <button
  ...
  return (
    ...
    >
      ...
      <div className={`p-4 border-b ${providedTheme.border} ${providedTheme.surfaceHover}`}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <select
              value={viewMode}
              onChange={(e) => setViewMode(e.target.value)}
              className={`px-3 py-1 rounded border ${providedTheme.input} ${providedTheme.border} text-sm`}
            >
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            <h4 className={`text-lg font-semibold ${providedTheme.text.primary}`}>
              {viewMode === 'week'
            ...
            <p className={`text-sm ${providedTheme.text.muted}`}>
      ...
      {showSettings && <SettingsModal onClose={() => setShowSettings(false)} themeClasses={providedTheme} />}
      {showFilter && <FilterModal onClose={() => setShowFilter(false)} themeClasses={providedTheme} />}
      {showExport && <ExportModal onClose={() => setShowExport(false)} themeClasses={providedTheme} />}
      {showImport && <ImportModal onClose={() => setShowImport(false)} themeClasses={providedTheme} />}
      {showAutoSchedule && <AutoScheduleModal onClose={() => setShowAutoSchedule(false)} themeClasses={providedTheme} />}
