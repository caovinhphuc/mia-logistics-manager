...
export const useFilters = () => {
  ...
  const getFilterSummary = useCallback(() => {
    ...
    if (filters.carrier.length > 0) activeFilters.push(`Nhà vận chuyển: ${filters.carrier.length}`);
    if (filters.destination.length > 0)
      activeFilters.push(`Đích đến: ${filters.destination.length}`);
    if (filters.product.length > 0) activeFilters.push(`Sản phẩm: ${filters.product.length}`);
    if (filters.category.length > 0) activeFilters.push(`Phân loại: ${filters.category.length}`);
  ...
  const applyFilters = useCallback(
    (items: InboundItem[]) => {
      return items.filter((item) => {
        ...
        // Carrier filter
        if (filters.carrier.length > 0 && item.carrier && !filters.carrier.includes(item.carrier)) {
          return false;
        }

        // Destination filter
        if (filters.destination.length > 0 && !filters.destination.includes(item.destination)) {
          return false;
        }
